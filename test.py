def launch_gui(inputs_dir: Path, outputs_dir: Path | None = None):
    ...
    def on_file_selected(event=None):
        ...
        try:
            ...
            # set calendar limit
            start_date_entry.config(mindate=tmin.date(), maxdate=tmax.date())
            end_date_entry.config(mindate=tmin.date(), maxdate=tmax.date())
    ...
    start_date_entry = DateEntry(frame_time, width=12, date_pattern="yyyy-mm-dd")
    start_date_entry.grid(row=0, column=1, padx=5, pady=5)

    ttk.Label(frame_time, text="Time (HH:MM):").grid(row=0, column=2)
    entry_start_hour = ttk.Entry(frame_time, width=3)
    entry_start_min = ttk.Entry(frame_time, width=3)
    entry_start_hour.grid(row=0, column=3, padx=2)
    ttk.Label(frame_time, text=":").grid(row=0, column=4)
    entry_start_min.grid(row=0, column=5, padx=2)

    # end date/time
    ttk.Label(frame_time, text="End date:").grid(row=1, column=0, padx=5, pady=5)
    end_date_entry = DateEntry(frame_time, width=12, date_pattern="yyyy-mm-dd")
    end_date_entry.grid(row=1, column=1, padx=5, pady=5)

    ttk.Label(frame_time, text="Time (HH:MM):").grid(row=1, column=2)
    entry_end_hour = ttk.Entry(frame_time, width=3)
    entry_end_min = ttk.Entry(frame_time, width=3)
    entry_end_hour.grid(row=1, column=3, padx=2)
    ttk.Label(frame_time, text=":").grid(row=1, column=4)
    entry_end_min.grid(row=1, column=5, padx=2)
